use scarscape::io::manifest::ManifestAdapter;
use scarscape::model::{SampleId, SampleInput};
...
fn manifest_from_reader_parses_records_into_domain_structs() {
    ...
    let it = ManifestAdapter::from_reader(&data[..]).unwrap();
    let records: Vec<SampleInput> = it.map(|r| r.unwrap()).collect();
...
fn manifest_missing_required_column_errors_immediately() {
    ...
    let err = ManifestAdapter::from_reader(&data[..]).unwrap_err();
